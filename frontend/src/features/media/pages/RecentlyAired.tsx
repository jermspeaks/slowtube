...
function RecentlyAired() {
  ...
  const loadEpisodes = async () => {
    try {
      ...
      const allEpisodes: Episode[] = (response.episodes || []).filter((ep) => {
        ...
        const dateStr = ep.air_date.includes("T")
          ? ep.air_date
          : `${ep.air_date}T00:00:00Z`;
      ...
      allEpisodes.sort((a, b) => {
        ...
        const dateStrA = a.air_date.includes("T")
          ? a.air_date
          : `${a.air_date}T00:00:00Z`;
        const dateStrB = b.air_date.includes("T")
          ? b.air_date
          : `${b.air_date}T00:00:00Z`;
  ...
  const handleTVShowClick = (tvShowId: number) => {
    navigate(`/media/tv-shows/${tvShowId}`);
  };
